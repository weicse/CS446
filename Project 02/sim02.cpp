...
unsigned int allocateMem(int maxMem){

	// Seed rand() with current time
	srand(time(0));
	// Generate random number for memory address
	unsigned int address = rand() % maxMem;
...
void* io_sim(void* casted_io){
	...
	// The package's char discerning between I and O is not used yet
	// Possibly implemented in future
	...
	while(!procInfo.empty()){
		...
		if(temp.getCode() == 'S'){
			...
			if(temp.getDescription() == "allocate"){
				...
				int memAddr = allocateMem(timeConf.getMem());
			...
			out1 << " - Process " << procNum << ": start " << temp.getDescription() << " input" << std::endl;
			out2 << " - Process " << procNum << ": start " << temp.getDescription() << " input" << std::endl;
			ioPackage op_data;
			...
			out1 << " - Process " << procNum << ": start " << temp.getDescription() << " output" << std::endl;
			out2 << " - Process " << procNum << ": start " << temp.getDescription() << " output" << std::endl;
			ioPackage op_data;
			if(temp.getDescription() == "hard drive"){
				op_data.io_time = hdT * temp.getCycles();
			}
